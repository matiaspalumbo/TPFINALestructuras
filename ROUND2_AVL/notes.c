...
Intervalo* union_ints(Intervalo* intv1, Intervalo* intv2) { // Une dos intervalos que se intersecan.
  Intervalo* intv = malloc(sizeof(Intervalo));
  intv->left = (intv1->left < intv2->left) ? intv1->left : intv2->left;
  intv->left = (intv1->right > intv2->right) ? intv1->right : intv2->right;
  return intv;
}
